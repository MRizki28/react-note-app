...
export default class EditNote extends Component {
  ...
  componentDidMount() {
    ...
    axios.get(`http://localhost:8000/api/note/${noteId}`)
      .then(response => {
  ...
  handleDelete(e) {
    e.preventDefault();
    //Make api call to delete the note with the id of this.props.match.params.id
    axios.delete(`http://localhost:8000/api/note/${this.props.match.params.id}`)
    .then(response => {
  ...
  handleSave(e) {
    e.preventDefault();
    //Api call to save of this.props.match.params.id
    axios.put(`http://localhost:8080/api/note/{${this.props.match.params.id}}`, {
  ...
  render() {
    ...
    return (
      ...
      <div className="px-4 mb-4">
          ...
          <textarea value={this.state.note} onChange={this.handleBodyChange.bind(this)}  type="text" className="w-full h-36 p-4 rounded-lg text-gray-300 bg-gray-800 outline-none bg-transparent mb-4" id="body" name="title" />
