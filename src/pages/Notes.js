...
export default class Notes extends Component {
  ...
  constructor(props){
    ...
    this.state = {
      notes :[
        { id: 1 , title: 'My First Note', body: 'keren sekali aku nya'},
        { id: 2 , title: 'My seccont Note', body: 'keren sekali aku nya'},
      ],
      ...
      <>
        ...
        <div className="px-4 mb-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
          {this.state.notes.filter(note => {
            return note.title.toLocaleLowerCase().match(this.state.searchQuery)
          }).map(note => (

              <a key={note.id} href="single-note.html" className="bg-blue-200 p-3 rounded shadow">
